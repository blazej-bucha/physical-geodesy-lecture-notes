...
\end{equation}
%
je \emph{prvá numerická excentricita}.
...
\begin{equation}
\label{eq:q0t}
Q_0(t) = \frac{1}{2} \, \ln\frac{1 + t}{1 - t} = \tanh^{-1}t{,}
\end{equation}
%
\begin{equation}
\label{eq:q1t}
Q_1(t) = \frac{t}{2} \, \ln\frac{1 + t}{1 - t} - 1 = t \, \tanh^{-1}t - 1{,}
\end{equation}
%
\begin{equation}
\label{eq:q2t}
Q_2(t) = \left( \frac{3}{4} t^2 - \frac{1}{4} \right) \, \ln\frac{1 + t}{1 - t} 
- \frac{3}{2}t = \left( \frac{3}{2} t^2 - \frac{1}{2} \right) \, \tanh^{-1}t 
- \frac{3}{2}t{,}
\end{equation}
%
kde
...
\begin{equation}
\label{eq:tanh}
\frac{1}{2} \, \ln \frac{1 + t}{1 - t} = \tanh^{-1}t{,}
\end{equation}
%
pričom $\tanh$ označuje hyperbolický tangens.  Funkcie $Q_{nm}(t)$ pre niekoľko 
prvých stupňov~$n$ a~rádov~$m$ sú znázornené na Obrázkoch~\ref{fig:lp_2nd} 
a~\ref{fig:lf_2nd} (porovnaj s Obrázkami~\ref{fig:lp} a~\ref{fig:lf}).  Funkcie 
$Q_0\left( z \right)$, $Q_1\left( z \right)$ a~$Q_2\left( z \right)$ 
komplexného argumentu~$z$ získame nahradením~výrazov~(\ref{eq:tanh}) vo 
vzťahoch~(\ref{eq:q0t}) až (\ref{eq:q2t}) výrazmi (porovnaj 
rovnice~\ref{eq:qn0_imag_def} a~\ref{eq:qn0_real_def})
%
\begin{equation}
\frac{1}{2} \, \ln \frac{z + 1}{z - 1} = \coth^{-1}z{.}
\end{equation}
...
V_\gidx(u, \beta, \lambda) = \frac{G \, M}{R} \, \sum_{n = 0}^\infty \sum_{k 
...
Teóriu normálneho tiažového poľa ekvipotenciálneho elipsoidu rozpracovali 
...
Na tomto mieste ešte zdôrazníme, že hoci budeme často hovoriť o~normálnom 
tiažovom poli \emph{Zeme}, je dobré mať na pamäti, že ide 
o~matematicko-fyzikálny model definovaný štyrmi základnými parametrami.  Všetky 
ostatné parametre, či už geometrické (napríklad lineárna excentricita) alebo 
fyzikálne (napríklad normálne tiažové zrýchlenie), sú odvoditeľné z~týchto 
...
V ďalších častiach tejto kapitoly sa budeme venovať tiažovému poľu 
ekvipotenciálneho eliposoidu.  Najprv podrobne odvodíme vzťahy na výpočet 
normálneho tiažového potenciálu pomocou rozvoja do radu sféroidických 
a sférických harmonických funkcií~(Kapitola~\ref{sec:normal_gravity_potential}) 
a následne, tentokrát už bez podrobného odvodenia, uvedieme vzťahy na výpočet 
normálneho tiažového zrýchlenia (Kapitola~\ref{sec:normal_gravity}).  Tieto 
poznatky budú dôležité neskôr v~Kapitole~\ref{sec:geoid_determination}, ktorá 
sa zaoberá určovaním geoidu.
...
vzťah je taký vzťah, ktorý má konečný počet členov.  Jeho výhodou je to, že 
často umožňuje jednoduchší a rýchlejší číselný výpočet so zadanou numerickou 
presnosťou v~porovnaní s~výpočtom nekonečného radu, ktorý je obmedzený na 
vhodne zvolený konečný počet členov.  V~tejto kapitole sa preto budeme snažiť 
nájsť uzavretý vzťah na výpočet normálneho tiažového potenciálu v~bodoch na 
povrchu ekvipotenciálneho elipsoidu a~nad ním.  Táto kapitola je spracovaná 
...
\emph{všeobecného} telesa.  Dôvodom je skutočnosť, že normálny tiažový 
potenciál na elipsoide má dve symetrie, jednu voči osi rotácie a druhú voči 
rovine rovníka.  Prvá symetria spôsobuje, že všetky nezonálne sféroidické 
harmonické koeficienty musia byť nulové, pretože tie závisia od dĺžky~$\lambda$ 
(pozri rovnicu~\ref{eq:vg_spheroidalh_no_norm}).  Druhá symetria spôsobuje, že 
nenulové môžu byť iba párne zonálne koeficienty (pozri vlastnosti sférických 
harmonických funkcií Kapitolu~\ref{sec:spherical_harmonics}).  Normálny tiažový 
potenciál môžeme teda zapísať v~tvare
%
\begin{equation}
\label{eq:u_sph}
\begin{split}
...
\end{equation}
%
kde $U_\gidx$ je normálny gravitačný potenciál a $U_\cidx$ je normálny 
odstredivý potenciál.  Pre lepšiu názornosť nasledujúcich krokov sme do vzťahu 
...
%
\begin{equation}
\frac{Q_n\left( i \dfrac{b}{E} \right)}{Q_n\left( i \dfrac{b}{E} \right)} = 1
\end{equation}
%
...
+ \frac{1}{2} \, \omega^2 \, (u^2 + E^2) \, \cos^2\beta = U_0{.}
\end{equation}
%
Symbol $U_0$ označuje normálny tiažový potenciál na ekvipotenciálnom elipsoide.  
Hodnota~$U_0$ je z definície konštantná (pozri 
Kapitolu~\ref{sec:equipotential_ellipsoid}).

Prostredná časť rovnice~(\ref{eq:u_sph_on_ell}) je formálne podobná 
vzťahu~(\ref{eq:f_synthesis}) s~dvoma rozdielmi.  Po prvé, je rovná konštante 
$U_0$ a nie funkcii $f(t)$, ktorá by závisela od $t = \sin\beta$.  Po druhé, 
druhý člen prostrednej časti rovnice obsahuje člen, ktorý, podobne ako 
Legendreove polynómy vo vnútri sumácie, závisí od $\sin\beta$, pretože 
$\cos^2\beta = \sqrt{1 - \sin^2\beta}$.  Prepíšme preto 
...
\end{equation}
%
ďalej vzťah pre Legendreov polynóm stupňa $n = 2$ (rovnica~\ref{eq:p0_to_p5})
%
\begin{equation}
P_2(\sin\beta) = \frac{3}{2} \, \sin^2\beta - \frac{1}{2}
\end{equation}
%
a rovnicu známu z trigonometrie
%
\begin{equation}
\sin^2 x + \cos^2 x = 1{.}
\end{equation}
...
vzťah~(\ref{eq:cnr}) môžeme prepísať do tvaru
%
\begin{equation}
\label{eq:cnr2}
\begin{split}
...
\end{equation}
%
Tento vzťah predstavuje rozvoj~(\ref{eq:f_synthesis}), pričom členy pred 
jednotlivými Legendreovými polynómami sú súradnice konštantnej funkcie~0 (pozri 
diskusiu o~súradniciach ku vzťahom~\ref{eq:r_synthesis}, \ref{eq:f_synthesis} 
a \ref{eq:f_sha_no_norm}).  Podobne ako rovnosť
%
\begin{equation}
\sum_{i = 1}^3 r_i \, \mathbf{e}_i = \mathbf{0}
\end{equation}
%
...
\end{equation}
%
Normálny gravitačný potenciál $U_\gidx(u, \beta)$ je teda daný vzťahom
%
\begin{equation}
...
\right)}{Q_2\left( i \dfrac{b}{E} \right)} \, C^{\mathrm{r}}_0 \, 
P_2(\sin\beta){,}
\end{equation}
%
ktorý môže byť dosadený do vzťahu~(\ref{eq:u_sph}) pre hľadaný normálny tiažový 
potenciál $U(u, \beta)$.
...
Kapitolu~\ref{sec:spheroidal_harmonics}), a tak vzťah pre normálny tiažový 
potenciál má konečný počet členov, čo bol cieľ vytýčený na začiatku tejto 
kapitoly.  Ďalšia dôležitá vlatnosť je tá, že vzťah~(\ref{eq:ug_spheroidal}) 
platí nielen na elipsoide pre $u = b$, ale aj pre $u \geq b$, teda kdekoľvek na 
povrchu ekvipotenciálneho elipsoidu a~v~jeho vonkajšom priestore.

\subsection{Okrajová úloha}
...
elipsoid.  Tento elipsoid sme prehlásili za ekvipotenciálny, čím sme každému 
bodu na jeho povrchu priradili hodnotu normálneho tiažového potenciálu~$U_0$.  
V súlade so Stokesovým teorémom~(Kapitola~\ref{sec:equipotential_ellipsoid}) 
sme následne na základe známej geometrie ekvipotenciálnej plochy a hodnoty 
normálneho tiažového potenciálu na nej dokázali zistiť, čomu je rovný normálny 
tiažový potenciál v~celom vonkajšom priestore tejto plochy.  Toto by, 
samozrejme, nebolo možné bez znalosti \emph{princípu}, akým sa mení gravitačný 
potenciál v~priestore.  Tento princíp je zachytený v~Laplaceovej rovnici (pozri 
Kapitolu~\ref{sec:meaning_of_laplace_equation_in_practice}).  O~sféroidických 
harmonických funkciách vieme, že sú riešením práve Laplaceovej rovnice 
v~redukovaných elipsoidických súradniciach.  Preto 
riešenie~(\ref{eq:ug_spheroidal}), ktoré je ich lineárna kombinácia s~rôznymi 
váhami (súradnicami $C_n^\mathrm{r}$), je taktiež harmonická funkcia, keďže 
Laplaceov operátor je lineárny (pozri rovnice~\ref{eq:laplace_additivity} 
...
elipsoidu) sa nazýva \emph{okrajová úloha}.

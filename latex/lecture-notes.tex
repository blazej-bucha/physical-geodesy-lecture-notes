...
funkcie stupňa $n$ a rádu $m$} a vzťah~(\ref{eq:vg_sh_no_norm}) predstavuje 
...
\label{fig:lf}
\centering
\includegraphics{../figs/legendre-functions.pdf}
...
Pozrime sa bližšie na grafickú reprezentáciu sférických harmonických funkcií 
(Obrázky~\ref{fig:sh} a \ref{fig:sh3d}).  Možno si všimnúť, že sférické 
harmonické funkcie rádu $k = 0$ striedavo rozdeľujú sféru v smere sférickej 
šírky na oblasti s kladnými hodnotami a na oblasti so zápornými hodnotami.  
Nezávisia tiež od sférickej dĺžky.  Tieto vlastnosti možno overiť dosadením 
rádu $k = 0$ do rovnice~(\ref{eq:ynk}),
%
\begin{equation}
\label{eq:yn0}
...
\end{equation}
%
Funkcia $Y_{n0}(\varphi, \lambda)$ mení $n$-krát znamienko na intervale 
$\varphi \in [-\frac{\pi}{2}, \frac{\pi}{2}]$ (porovnaj rovnicu~\ref{eq:yn0} 
a Obrázok~\ref{fig:lp}).  Možno si tiež všimnúť, že parita stupňa $n$ určuje 
symetriu funkcie $Y_{n0}(\varphi, \lambda)$ voči rovine rovníka; ak $n = 0, 2, 
4, \dots$, sférická harmonická funkcia je symetrická voči rovine rovníka 
a nesymetrická ak $n = 1, 3, 5, \dots$ (pozri 
Kapitolu~\ref{sec:legendre_polynomials}). Sférická harmonická funkcia rádu $k 
= 0$ sa nazýva \emph{zonálna sférická harmonická funkcia}.

Ak $0 < |k| < n$, sférická harmonická funkcia mení znamienko s oboma 
súradnicami.  V smere sférickej šírky dochádza k $n - |k|$ zmenám znamienka 
a v smere sférickej dĺžky sa znamienko mení $2|k|$-krát.  Počet $n - |k|$ 
vyplýva zo skutočnosti, že Legendreova funkcia $P_{n|k|}(\sin\varphi)$ súvisí 
s $|k|$-tou deriváciou Legendreovho polynómu $P_n(\sin\varphi)$ (pozri 
rovnice~\ref{eq:pnm_def} a \ref{eq:pnm_ferrer}).  Deriváciou polynómu sa 
znižuje jeho stupeň, teda aj počet miest, v ktorých funkcia môže pretínať 
vodorovnú os.  Počet nulových hodnôt v smere sférickej dĺžky $2|k|$ vyplýva 
z vlastností trigonometrických funkcií $\cos(k\lambda)$ a $\sin(|k|\lambda)$.  
Sférická harmonická funkcia rádu $0 < |k| < n$ má prívlastok \emph{teserálna}.

Na záver, ak $|k| = n$, sférická harmonická funkcia mení znamienko $2|k|$-krát 
v smere sférickej dĺžky, no $0$-krát v smere sférickej šírky, pretože $n - |k| 
= n - n = 0$.  Takáto funkcia sa nazýva \emph{sektorálna sférická harmonická 
funkcia}.

V Kapitole~\ref{sec:unit_vectors} sme ukázali, že ľubovoľný vektor $\vec r$ 
z trojrozmerného priestoru je možné popísať pomocou jeho súradníc $r_i$, $i 
= 1, 2, 3$.  Sférické harmonické funkcie majú podobné využitie ako jednotkové 
vektory, avšak v priestore funkcií na sfére.  Je možné ukázať, že pre ľubovoľnú 
spojitú funkciu $f(\varphi, \lambda)$ platí
%
\begin{equation}
f_{nk} = \frac{1}{4\pi} \iint_{\sigma} f(\varphi, \lambda) \, Y_{nk}(\varphi, 
\lambda) \, \diff \sigma{,}
\end{equation}
%
\begin{equation}

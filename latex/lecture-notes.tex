...
z rovnice~(\ref{eq:vg_body}).  Zavedených bude niekoľko nových pojmov, ktoré si 
žiadajú podrobnú diskusiu.  Aby sme však pričasto neprerušovali prirodzený 
myšlienkový postup odvodenia, nové pojmy predstavíme len vo forme nevyhnutnej 
na pochopenie odvodenia samotného.  Vrátime k nim však v nasledujúcich 
kapitolách, podobne ako sa čitateľ môže vrátiť k tejto kapitole po prečítaní 
dovysvetľujúcich kapitol.
...
Legendreove polynómy}.  Derivovaním generickej funkcie podľa $\alpha$, 
dosadením $\alpha = 0$ do výsledku derivovania a následným dosadením získaného 
výrazu do~(\ref{eq:pn}) sa možno presvedčiť, že Legendreove polynómy stupňov $n 
= 0, 1, 2$ sú rovné
%
\begin{equation}
\label{eq:p0_p1_p2}
...
\end{equation}
%
Legendreove polynómy je možné vypočítať aj Rodriguesovým vzorcom 
\citep[napríklad][]{SansoGeoidDetermination},
%
\begin{equation}
...
\end{equation}
%
Grafické znázornenie Legendreových polynómov stupňov $n =0, 1, \dots, 5$ je 
...
$n$.  Po uvážení vzťahu~(\ref{eq:pn_rodrigues}) môžeme teda upraviť 
rovnicu~(\ref{eq:pnm_def}) nasledovne,
%
\begin{equation}
...
\end{equation}
%
Vo vzťahu~(\ref{eq:pnm_def}) pribudol ešte člen $\delta_{m0}$, ktorý má názov 
Kroneckerovo delta.  Všeobecná definícia pre $m$ a $l$ má tvar
%
\begin{equation}
\delta_{ml} =
%
...
Legendreové polynómy a Legendreove funkcie 
(Kapitola~\ref{sec:legendre_polynomials_and_legendre_functions}), sférické 
harmonické funkcie~(Kapitola~\ref{sec:spherical_harmonics}) a na záver samotný 
...
\label{sec:legendre_polynomials_and_legendre_functions}
...
niektorých systémov funkcií popísať i každú spojitú funkciu $f(t)$ na intervale 
$t \in [1, 1]$.  Jeden z takýchto systémov funkcií predstavujú Legendreove 
polynómy~$P_n(t)$.
...
pre ľubovoľné $t \in [-1, 1]$.  Vzťah~(\ref{eq:f_synthesis}) je teda možné 
vnímať aj ako analógiou vzťahu~(\ref{eq:r_synthesis}).  Súradnice $f_n$ sú dané 
skalárnym súčinom funkcií $f(t)$ a $P_n(t)$ (porovnaj s~\ref{eq:r_analysis}),
%
\begin{equation}
\label{eq:f_analysis}
...
nájsť napríklad v \cite{Freeden2009} či \cite{SansoGeoidDetermination}.

Pokračujme v analógii s jednotkovými vektormi.  Podobne ako skalárny súčin 
dvoch rôznych jednotkových vektorov je nulový,
%
\begin{equation}
\vec e_i \cdot \vec e_j = 0{,} \quad i \neq j{,}
\end{equation}
%
podobne je nulový i skalárny súčin dvoch rôznych Legendreovych polynómov,
%
\begin{equation}
...
Legendreove polynómy sú \emph{ortogonálne} na intervale $t \in [-1, 1].$  
...
Numerický výpočet Legendreových polynómov je zvyčajne najvýhodnejšie realizovať 
nie rovnicami~(\ref{eq:pn}) či (\ref{eq:pn_rodrigues}), ale rekurentnými 
vzťahmi.  Jeden z najznámejších vzťahov je Bonnetov rekurentný vzťah,
%
\begin{equation}
\label{eq:pn_bonnet}
...
\end{equation}
%
Použitím začiatočných hodnôt $P_0(t) = 0$ a $P_1(t) = 1$ (pozri 
rovnicu~\ref{eq:p0_p1_p2}) je možné vzťahom~(\ref{eq:pn_bonnet}) vypočítať 
Legendreov polynóm pre ľubovoľné $n > 1$ a $t \in [-1, 1]$.
...
\end{equation}
%
kde $P_n'(t) = \diff P_n(t) \slash \diff t$.  Existuje široká škála ďalších 
vzťahov pre Legendreove polynómy, ich derivácie či integrály 
\citep[napríklad][]{Gradshteyn2007,Freeden2009,Olver2010}.
...
language=mypython, label=src:lp, 
...
harmonických funkcií, language=mypython, 
captionpos=t]{../python/spherical-harmonics.py}

...
demonštrujúci diskutovanú problematiku.  Nádejame sa, že vlastným štúdiom,
...
nie sú, no mohli či mali byť diskutované.  Skôr než za uzavretý študijný
...
spojnicou hmotných bodov \citep{Kellogg1967}.  \emph{Hmotný bod} je 
...
symbolom $\vec v(t_0)$.  Predpokladajme, že planéta je v pohybe, teda $\| \vec 
v(t_0) \| > 0$.  Ak by od času $t_0$ nepôsobila na planétu žiadna sila, 
\emph{Newtonov pohybový zákon} hovorí, že planéta by sa ďalej pohybovala 
rovnomerným priamočiarym pohybom v~smere vektora $\vec v(t_0)$.  Za istý čas by 
sa teda dostala do bodu $P'(t_1)$.  Jediný spôsob ako zmeniť túto dráhu je 
pôsobením sily na planétu.  Z~rovnice~(\ref{eq:newton_law}) vyplýva, že v~čase 
$t_0$ pôsobí na planétu gravitačnou silou $\vec F_\gidx(t_0)$ Slnko, pričom 
smer sily je daný spojnicou planéty a~Slnka.  Gravitačná sila preto pritiahne 
planétu v~smere vektora $\vec F_\gidx(t_0)$, čím zmení hypotetickú trajektóriu, 
ktorou by sa planéta pohybovala bez pôsobenia tejto sily.  V~kombinácii 
s~vektorom rýchlosti gravitačná sila spôsobí, že planéta sa dostane do bodu 
$P(t_1)$.  V~čase $t_1$ sa opísaná situácia opakuje, pričom vektory rýchlosti 
a~sily už majú iný smer a~s~vysokou pravdepodobnosťou aj inú veľkosť.  
...
bodmi, a hmota týchto elementov je koncentrovaná do jedného bodu
\citep{Kellogg1967}.
...
prevzaté z~modelu \texttt{grav-sr-2arcsec} \citep{GravSR2arcsec}.}
...
zákon zhruba o~jedno storočie \citep{MacMillan1930,Jekeli2015}.  Fenomén 
...
S~konceptom potenciálu poľa prišiel podľa \cite{MacMillan1930} taliansky 
...
sa rozličnými zdrojmi poľa (hmotný bod, hmotná priamka, nekonečne tenká vrstva,
všeobecné teleso a~pod.) s~rôznym priestorovým rozložením hustôt (konštantná,
premenlivá, kladná, dokonca i~záporná a~pod.) a~neobmedzuje sa iba na
gravitačné pole.  Táto úloha je nesmierne náročná.  Čo sa napríklad stane
...
sa nazýva \emph{Coriolisova sila} \citep{Torge1989,SansoGeoidDetermination}.  
Azda väčšina geodetických meracích
...
\citep{Torge1989,SansoGeoidDetermination}.  Tieto tri
...
tiažového zrýchlenia $\| \vec g(P) \|$ sa nazýva \emph{gravimeter}.  Vedný 
odbor zaoberajúci sa meraním tiažového zrýchlenia sa nazýva \emph{gravimetria} 
(pozri napríklad \citealt{Torge1989,Rozimant1994} či \citealt{Janak2010}).  
...
potenciál jednotky $\mathrm{m}^2 \ \mathrm{s}^{-2}$.  Rozdiel tiažového 
potenciálu medzi dvoma bodmi sa niekedy udáva aj v~geopotenciálnych jednotkách,
%
\begin{equation}
\label{eq:gpu_unit}
...
\end{equation}
%
Presnosť určenia rozdielu tiažového potenciálu je približne $0.1\ \mathrm{gal} 
\ \mathrm{m}$ na vzdialenosť jedného kilometra \citep{MoritzPhysicalGeodesy}.
...
vlastností ekvipotenciálnych plôch, napríklad ich krivostí, je možné nájsť
napríklad v~prácach \cite{Janak2006} a~\cite{MoritzPhysicalGeodesy}.
...
technikách je možné nájsť napríklad v~prácach \cite{SeeberSatelliteGeodesy}
a~\cite{MoritzPhysicalGeodesy}.
...
konštantná v~každom jeho bode (pozri Kapitoly~\ref{sec:introduction} 
a \ref{sec:equipotential_surface}).  Túto vlastnosť by bolo výhodné zužitkovať 
aj pri voľbe normálneho tiažového poľa.  Budeme preto požadovať, aby 
...
približné hodnoty na linearizáciu úloh spojených s určovaním geoidu.  Dvojosový 
rotačný elipsoid s~konštantným normálnym tiažovým potenciálom na svojom povrchu 
sa nazýva \emph{ekvipotenciálny elipsoid}.  V~súčasnosti je to najčastejší 
spôsob voľby normálneho tiažového poľa Zeme.
...
sféroid} \citep{Moritz1967}.  V~oboch prípadoch sú koeficienty stupňa~1 nulové, 
pretože začiatok súradnicového systému je vložený do ťažiska Zeme 
...
a získaná dodatočná presnosť zväčša nie je významná \citep{Moritz1967}.
...
znalosť hustoty v~normálnom telese potrebná \citep{Karcol2017}.  Zdá sa, že 
...
\cite{MoritzTheFigureOfTheEarth}, \cite{TorgeGeodesy}, \cite{Conway2000} či 
\cite{Karcol2017}.
...
\emph{Stokesov--Poincarého} teorém hovorí, že ak teleso o~celkovej 
hmotnosti~$M$ rotuje konštantnou uhlovou rýchlosťou~$\omega$ okolo pevnej osi, 
a ak $S$ je ekvipotenciálna plocha jeho tiažového poľa obklopujúca celé teleso, 
potom tiažový potenciál vo vonkajšom priestore plochy~$S$ je jednoznačne určený 
konštantami $M$ a $\omega$ a parametrami definujúcimi plochu~$S$ 
\citep{TorgeGeodesy}.
...
Union of Geodesy and Geophysics}, IUGG) a Medzinárodnou asociáciou geodézie 
(angl. \textit{International Association of Geodesy}, IAG) na 17.~generálnom 
zasadnutí IUGG v Canberra v decembri 1979.  Dôležité je spomenúť, že použitá 
hodnota konštanty $GM$ zahŕňa hmotnosť Zeme \emph{a} hmotnosť zemskej atmosféry 
a koeficient $J_{2,0}$ nezahŕňa účinok permanentnej slapovej deformácie.  Medzi 
koeficientmi $J_{2,0}$, $C_{2,0}$ a $\bar{C}_{2,0}$ platia nasledovné vzťahy 
\citep{Moritz1967,MoritzPhysicalGeodesy},
%
\begin{equation}
C_{2,0} = -J_{2,0}{,}
\end{equation}
...
vychádza z~GRS80.  Z hľadiska tvaru sú si oba elipsoidy podobné; dĺžka ich 
hlavnej polosi je $6\, 378\, 137\ \mathrm{m}$ a dĺžka vedľajšej polosi sa 
odlišuje iba o~desatinu milimetra, $b^{\mathrm{GRS80}} = 6\, 356\, 752.3141\ 
\mathrm{m}$ voči $b^{\mathrm{WGS84}} = 6\, 356\, 752.3142\ \mathrm{m}$ 
\citep{MoritzPhysicalGeodesy}.  Z fyzikálneho hľadiska je už medzi elipsoidmi 
rozdiel väčší.  Uhlová rýchlosť rotácie oboch elipsoidov je síce rovnaká, no 
odlišné hodnoty geocentrických gravitačných konštánt spôsobujú rozdiel 
v~normálnom tiažovom potenciáli na elipsoide, $U_{0}^{\mathrm{GRS80}} = 62\, 
636\, 860.850\ \mathrm{m}^2 \, \mathrm{s}^{-2}$ a $U_{0}^{\mathrm{WGS84}} 
= 62\, 636\, 851.715\ \mathrm{m}^2 \, \mathrm{s}^{-2}$ 
\citep{MoritzPhysicalGeodesy}.  Rozdiel $|U_0^{\mathrm{GRS80}} 
- U_0^{\mathrm{WGS84}}|$ je teda približne o~5 rádov väčší ako presnosť určenia 
...
napríklad v~prácach \cite{TorgeGeodesy} a~\cite{MoritzPhysicalGeodesy}.

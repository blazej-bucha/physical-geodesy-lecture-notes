...
V~niektorých situáciách je potrebné skúmať gravitačné pole telies, ktoré sú 
sploštené.  Príkladom splošteného telesa je samotná Zem či mnohé asteroidy 
Slnečnej sústavy (napríklad Eros, Castalia či Bennu; pozri \cite{Garmier2001}, 
\cite{Hu2015}, \cite{Sebera2016} či \cite{Reimond2016}).  Ak by sme rozvinuli 
gravitačný potenciál týchto telies do radu sférických harmonických 
...
problematike z~Kapitoly~\ref{sec:spherical_harmonic_expansion}.

\begin{figure}
\centering
\input{./fig-spheroidal-harmonics-convergence.pdf_tex}
\caption{Sféra a elipsoid konvergencie harmonických 
radov~(\ref{eq:vg_sh_no_norm}) a~XX.  Oba rady konvergujú vo vonkajšom 
priestore svojej plochy konvergencie, no v~jej vnútri ich konvergencia zaručená 
nie je.  Oblasť konvergencie sféroidického harmonického radu je teda väčšia 
o~vyšrafovanú oblasť.}
...
a jedným z ohnísk elipsy (Obrázok~\ref{fig:reduced_ell_coords}),
%
\begin{equation}
E = \sqrt{a^2 - b^2}{.}
\end{equation}
...
diferenciálnej rovnice vo sférických 
súradniciach~(rovnica~\ref{eq:vg_laplace_sph}).  Cieľom takéhoto prístupu je 
nájsť tri funkcie, $f(r)$, $g(\varphi)$ a $h(\lambda)$, z~ktorých každá je 
funkciou práve jednej sférickej súradnice.  Hľadané funkcie majú tvar (porovnaj 
...
riešenie využíva na rozvoj \emph{harmonickej} funkcie vo vnútri sféry 
\parencite{MoritzPhysicalGeodesy}.  Takáto situácia však nie je predmetom tejto 
práce.

Ak chceme získať sféroidické harmonické súradnice, jeden z~možných prístupov je 
práve riešenie Laplaceovej diferenciálnej rovnice, tentokrát však 
v~redukovaných elipsoidických súradniciach.  Bez podrobnejšieho odvodenia 
uvedieme tvar Laplaceovej diferenciálnej rovnice pre gravitačný potenciál 
v~redukovaných elipsoidických súradniciach \parencite{MoritzPhysicalGeodesy},
%
\begin{equation}
...
\end{equation}
%
Pre reálny argument $t \in [-1, 1]$ je definovaná vzťahom
\parencite{MoritzPhysicalGeodesy}
%
\begin{equation}
...
o~funkcie s~odlišným správaním v porovnaní s~$P_{nm}(t)$.  Obrázky~XX a~YY  
znázorňujú funkcie $Q_{nm}(t)$ pre niekoľko prvých stupňov~$n$ a~rádov~$m$ 
(porovnaj s Obrázkami~\ref{fig:lp} a~\ref{fig:lf}).  Rovnaké rekurentné 
vzťahy~(\ref{eq:pn_bonnet}) ako pre~$P_n(t)$ platia aj pre~$Q_n(t)$ 
\parencite{MoritzPhysicalGeodesy}.  Podobne ako funkcie $P_{nm}(t)$, i~funkcie 
$Q_{nm}(t)$ vyhovujú Laplaceovej diferenciálnej rovnici druhého rádu 
\parencite[pozri rovnicu
\ref{eq:legfunc1_differential_equation};][]{MoritzPhysicalGeodesy}.


\subsection{Numerický výpočet Legendreových funkcií drúheho druhu}

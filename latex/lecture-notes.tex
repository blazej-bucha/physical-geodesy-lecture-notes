...
Hoci nedokážeme priamo odmerať tiažový potenciál, \emph{rozdiel} tiažového 
potenciálu určený môže byť, a~to nepriamo kombináciou gravimetrie a nivelácie.
...
W \cdot \diff \vec x = \vec g \cdot \diff \vec x{,}
\end{equation}
%
kde
%
...
\end{equation}
%
a symbol $\cdot$ označuje skalárny súčin dvoch vektorov.
...
diferenciál $\diff W$ je nulový.  To znamená, že skalárny súčin~vektorov $\vec 
g$ a $\diff \vec x$ je v~takom prípade nulový, a teda dokázali sme, že 
vektor~$\vec g$ je kolmý na ekvipotenciálnu plochu.

Ak dochádza k~diferenciálnej zmene polohy $\diff \vec x$ pozdĺž tiažnice smerom 
od geocentra, potom medzi vektormi $\vec g$ a $\diff \vec x$ je uhol 
$180^{\circ}$ a totálny diferenciál tiažového potenciálu je daný vzťahom
%
\begin{equation}
...
\end{equation}
%
kde $H$ je diferenciálna dĺžka tiažnice.  Integráciou 
...
\end{equation}
%
V~súčasnosti dosiahnuteľná presnosť určenia rozdielu tiažového potenciálu je 
približne $0.1\ \mathrm{gal} \ \mathrm{m}$ na vzdialenosť jedného kilometra 
\parencite{MoritzPhysicalGeodesy}.
...
aplikácie je výhodné upraviť rovnice~(\ref{eq:geopotential_number}) 
a (\ref{eq:orthometric_height_1}) nasledovne \parencite{MoritzPhysicalGeodesy},
%
\begin{equation}
...
bodom.  Rovnica~(\ref{eq:orthometric_height_2}) definuje ortometrickú výšku (na 
Obrázku~\ref{fig:equipotential_surfaces} dĺžka tiažnice od geoidu po bod~$P$).
...
posledná menovaná norma, preto ju bližšie popíšeme.

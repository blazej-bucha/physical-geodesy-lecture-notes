...
Cieľom tejto kapitoly je odvodiť vzťah~(\ref{eq:vg_synthesis}) 
z rovnice~(\ref{eq:vg_body}).  Zavedených bude niekoľko nových pojmov, ktoré 
samé o sebe si vyžadujú podrobnú diskusiu.  Aby sme však pričasto neprerušovali 
prirodzený myšlienkový postup odvodenia, nové pojmy predstavíme v tejto 
kapitole len vo forme nevyhnutnej na pochopenie odvodenia samotného.  Vrátime 
k nim však v nasledujúcich kapitolách, podobne ako sa čitateľ môže vrátiť 
...
Legendreove polynómy.  Derivovaním generickej funkcie podľa $\alpha$, dosadením 
$\alpha = 0$ do výsledku derivovania a následným dosadením získaného výrazu 
do~(\ref{eq:pn}) sa možno presvedčiť, že Legendreove polynómy stupňov $n = 0, 
1, 2$ sú rovné
%
\begin{equation}
\begin{split}
...
\end{equation}
%
Legendreove je možné vypočítať aj Rodriguesovým vzorcom 
...
\end{equation}
%
So zvyšujúcim sa stupňom $n$ sa však oba spôsoby výpočtu stávajú nepraktické.  
V Kapitole~\ref{sec:legendre_polynomials_and_legendre_functions} budú preto 
ukázané efektívnejšie spôsoby výpočtu Legendreových polynómov.
...
vynecháme, no je možné vyhľadať ho napríklad v sekcii~90 Kapitoly~IV knihy 
\cite{Hobson}.

Dekompizičný vzorec pre Legendreove polynómy možno zapísať vo viacerých 
...
\begin{equation}
\label{eq:pnm_def}
P_{nm}(t) = (1 - t^2)^{\frac{m}{2}} \, \frac{\diff^m P_n(t)}{\diff t^m}{,}
\end{equation}
%
kde symbol $m$ môže nadobúdať hodnoty $m = 0, 1, \dots, n$ (pozri druhú sumáciu 
vo vzťahu~\ref{eq:pn_decomposition_formula}) a nazýva sa \emph{rád}.  Ak je rád 
nulový, zvykne sa v zápise vynechať, teda $P_n(t) = P_{n0}(t)$.  Zo 
vzťahu~(\ref{eq:pnm_def}) vidíme, že Legendreova funkcia stupňa $n$ a rádu $m$ 
súvisí s $m$-tou deriváciou Legendreoveho polynómu stupňa $n$.  Vo 
vzťahu~(\ref{eq:pnm_def}) pribudol ešte člen $\delta_{m0}$, ktorý má názov 
Kronekerovo delta a je daný podmieneným spôsobom,
%
\begin{equation}
\delta_{m0} =
%
\begin{cases}
1{,} \quad \mathrm{ak} \quad m = 0{,}\\
...
v (takmer) ľubovoľnom bode $P$.  Nepripomína to situáciu 
...
v skutočnosti súradnicami $v_{nm}$ gravitačného potenciálu 
